...
def main(args):
    ...
    if (not Options.in_pdb or not Options.in_span):
    ...
    with  open(Options.lip_has_pore, 'r') as fp:
        lip_has_pore_dic = json.loads(fp.read())

    mutations = []
    mutate = {}
    if Options.mutfile != 'False':
        with open(Options.mutfile, 'r') as f:
