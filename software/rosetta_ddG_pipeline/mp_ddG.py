...
def postprocess_rosetta_ddg_mp_pyrosetta(folder, output_name='ddG.out', sys_name='', uniprot=''):
    ddg_file = os.path.join(folder.ddG_run, output_name)
    prims_file = os.path.join(folder.ddG_output, f'prims_rosetta_XXX_{sys_name}.txt')
    ...
    rosetta_to_prism(ddg_file, prims_file, sequence, rosetta_info=None,
                     version=1, uniprot=uniprot, sys_name=sys_name)
