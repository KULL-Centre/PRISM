    ...
    for i in range(1, len(fasta_seq.strip())+1):
        ...
        for AA in refs:
            ...
            if a in str(list_keys):
                ...
                line.append('-')  
        scorefile.write(scorefile_line.format(i, line[0+(i-1)*20], line[1+(i-1)*20], line[2+(i-1)*20], line[3+(i-1)*20], line[4+(i-1)*20], line[5+(i-1)*20], line[6+(i-1)*20], line[7+(i-1)*20], line[8+(i-1)*20], line[9+(i-1)*20], line[10+(i-1)*20], line[11+(i-1)*20], line[12+(i-1)*20], line[13+(i-1)*20], line[14+(i-1)*20], line[15+(i-1)*20], line[16+(i-1)*20], line[17+(i-1)*20], line[18+(i-1)*20], line[19+(i-1)*20]))
...
if __name__ == '__main__':
    parse_rosetta_ddgs(sys_name=sys.argv[1], chain_id=sys.argv[2], fasta_seq=sys.argv[3], out_path=sys.argv[4])
