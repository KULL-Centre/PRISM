...
try:
    ...
    from prism2mutfile import residue_files, combined_mut, pipeline_combined_mut
...
def create_homodimer_input(pdb_file, prism_file, output_dir, additional=False, inclWT=True):
    ...
    if additional:
        residue_files(output_dir, df, inclWT=inclWT, drop_multi_mut=False)
        pipeline_combined_mut(output_dir, df, inclWT=inclWT, drop_multi_mut=False)
    combined_mut(output_dir, df, inclWT=inclWT, drop_multi_mut=False)
