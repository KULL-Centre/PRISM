...
def extract_uniprot_info(uniprot_id, reviewed='*'):
	...
	#I think this part just generates the request from the search string
	req2 = urllib.request.Request(search_string)
		
	#I didn't get which part of urllib sends the actual server request, I think it's this one. 
	...
	#if we arrive here the counter is > 3 and we have tried 3 times unsuccessfully to query uniprot
	print("Failed", counter," times to query uniprot for", uniprot_id, ", check what's up")
	return
